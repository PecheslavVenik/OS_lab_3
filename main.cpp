...
void log_msg(const char *fmt, ...) {
    ...
    if (f) {
        fprintf(f, "[%s] [PID:%d] ", tbuf, g_my_pid);
        va_list ap;
        va_start(ap, fmt);
        vfprintf(f, fmt, ap);
        va_end(ap);
...
void* spawner_thread(void* arg) {
    ...
    while (g_running) {
        ...
        Sleep(3000);
        WaitForSingleObject(hCounterMutex, INFINITE);
        bool busy1 = g_area->child1_running, busy2 = g_area->child2_running;
        if (!busy1) {
...
void run_child(int child_no) {
    ...
    if (child_no == 1) {
        ...
        WaitForSingleObject(hCounterMutex, INFINITE);
#else
        pthread_mutex_unlock(counter_mutex); sleep(2); pthread_mutex_lock(counter_mutex);
#endif
        g_area->counter /= 2;
...
void init_shm() {
    ...
    bool newly_created = (st.st_size == 0);
    if (newly_created) { ftruncate(shm_fd, sizeof(SharedArea)+3*sizeof(pthread_mutex_t)); }
    void* area = mmap(NULL, sizeof(SharedArea)+3*sizeof(pthread_mutex_t), PROT_READ|PROT_WRITE, MAP_SHARED, shm_fd, 0);
...
void become_leader() {
#ifdef _WIN32
    if (WaitForSingleObject(hLeaderMutex, 0) == WAIT_OBJECT_0) {
        g_is_leader = true;
...
#else
    if (pthread_mutex_trylock(leader_mutex) == 0) {
        g_is_leader = true;
        g_area->leader_pid = g_my_pid;
    } else {
        g_is_leader = false;
...
int main(int argc, char **argv) {
...
#ifndef _WIN32
        if (!g_is_leader) {
            for (int i=0; i<20 && !g_is_leader; ++i) { usleep(200000); become_leader(); }
            if (!g_is_leader) { fprintf(stderr, "Не удалось захватить лидерство, выхожу\n"); return 1; }
        }
#endif
    }
    ...
    if (g_is_leader) {
#ifdef _WIN32
        HANDLE t2 = CreateThread(NULL, 0, log_thread, NULL, 0, NULL);
        HANDLE t3 = CreateThread(NULL, 0, spawner_thread, argv, 0, NULL);
#else
        pthread_t t2, t3;
